...
class TestCommissionUnits(FrappeTestCase):
    def setUp(self):
        ...
        self.non_serial_code = "RFID-NON-SERIAL"
        ...
        # A NON-serialized item to exercise the has_serial_no == 0 rejection branch.
        if not frappe.db.exists("Item", self.non_serial_code):
            frappe.get_doc({
                "doctype": "Item", "item_code": self.non_serial_code,
                "item_name": self.non_serial_code, "item_group": "All Item Groups",
                "stock_uom": "Nos", "is_stock_item": 1, "has_serial_no": 0,
            }).insert(ignore_permissions=True)

    def test_creates_serials_and_is_idempotent(self):
        # The endpoint commits, so FrappeTestCase's per-test rollback cannot undo the
        # created Serial Nos. Use fresh EPCs each run so the test stays repeatable.
        suffix = frappe.generate_hash(length=8).upper()
        epcs = ["BE3A0001" + suffix + "01", "BE3A0001" + suffix + "02"]
    ...
    def test_rejects_non_serialized_item(self):
        with self.assertRaises(frappe.ValidationError):
            commission_units(self.non_serial_code, self.warehouse, json.dumps(["BE00"]))
